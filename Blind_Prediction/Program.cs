        ...
        static void Draw(EventArgs e)
        {
            ...
            foreach (var v in HeroList)
            {
                if (!v.Key.IsVisible)
                {
                    ...
                    Drawing.DrawRect(Drawing.WorldToScreen(ePos), new Vector2(50, 50), Drawing.GetTexture("materials/ensage_ui/miniheroes/" + v.Key.Name.Substring(v.Key.Name.LastIndexOf('_') + 1) + ".vmat"));
                }
            }
        }
            ...
            foreach (var v in HeroList)
            {
                if (!v.Key.IsVisible)
                {
                    Vector3 ePos = new Vector3(v.Key.NetworkPosition.X + (float)Math.Cos(v.Key.NetworkRotationRad) * v.Key.MovementSpeed * (Game.GameTime - v.Value),
                                                ...
                                                v.Key.NetworkPosition.Z + 50);
                    if (v.Key.NetworkActivity != NetworkActivity.Move) ePos = v.Key.NetworkPosition;
                    Text.DrawText(null, FirstCharToUpper(v.Key.Name.Substring(v.Key.Name.LastIndexOf('_') + 1)),
                                        (int)Math.Round(WorldToMiniMap(ePos).X, MidpointRounding.AwayFromZero) - (int)(v.Key.Name.Substring(v.Key.Name.LastIndexOf('_') + 1).Length * 3 * ScaleX),
